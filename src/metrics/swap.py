...
class SWAP:
    ...
    def register_hook(self, model):
        for n, m in model.named_modules():
            if isinstance(m, nn.ReLU) or isinstance(m, GELUActivation) or isinstance(m, nn.GELU) or isinstance(m, nn.SiLU):
    ...
    def forward(self):
        ...
        with torch.no_grad():
            if type(self.inputs) != torch.Tensor:
                ...
                activtions = torch.cat([f.view(self.inputs['input_ids'].shape[0], -1) for f in self.interFeature], 1)
            else:
                self.model.forward(self.inputs.to(self.device))
                activtions = torch.cat([f.view(self.inputs.shape[0], -1) for f in self.interFeature], 1)
